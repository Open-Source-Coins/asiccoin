...
// Copyright (c) 2013 AsicCoin developers
...
const string strMessageMagic = "AsicCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	if(rand > 60000 && rand < 70001)			
	{
		nSubsidy = 2048 * COIN;
	}
	else if(rand > 33000 && rand < 34001)	
	{
		nSubsidy = 8096 * COIN;
	}
	else if(rand > 77770 && rand < 77781)	
	{
		nSubsidy = 65536 * COIN;
	}

	// 1st week is bonus week
	if(nHeight < 5761)		// 1st 2 days x5
	{
		nSubsidy *= 5;
	}
	else if(nHeight < 11521)	// next 2 days x3
	{
		nSubsidy *= 3;
	}
	else if(nHeight < 20161)	// next 3 days x2
	{
		nSubsidy *= 2;
	}
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AsicCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AsicCoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running AsicCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AsicCoinMiner terminated\n");
        throw;
    }
}
